...
void printCrs(const Cpnode CrsList){
	...
	while (pCrs != NULL) {
		wprintf(L"课程信息:%d\t%s\t%d\t%d\t%.1lf\t%.2lf\t%.1lf\t%.2lf\n\n学生：\n",
			pCrs->cnum,
			pCrs->cname,
			pCrs->character,
			pCrs->headcount,
...
void testUI() {
	...
	Button exitButton(500, 500, 200, 60, L"退出", LIGHTBLUE, WHITE);
	...
	while (!_kbhit()) {
		...
		if (peekmessage(&msg, -1, true)) {
			...
			if (button2.mouseClick(msg)) {
				wprintf(L"press button2\n");
				wprintf(L"box2 is:%s\n", box2.text);
			}
